...
df_Seats = pd.melt(df, id_vars = ['Hand_ID', 'BigBlind'
#                                  , 'Hour_Day'
#                                  , 'Pot', 'Rake'
                                  ],
                     value_vars = ['Seat_1', 'Seat_2', 'Seat_3', 'Seat_4', 'Seat_5', 'Seat_6'],
...
df2 = pd.concat([df_Seats, df_Stacks, df_Pos, df_CallPRE, df_RaisePRE
#                 , df_FlopBet, df_FlopCall, df_FlopRaise
                 , df_SawFLop
#                 , df_TurnBet, df_TurnCall, df_TurnRaise
                 , df_SawTurn
#                 , df_RiverBet, df_RiverCall, df_RiverRaise
                 , df_SawRiver, df_Winnings, df_SD, df_Winner], axis = 1)
...
# =============================================================================
# % of win
dfWon = df2[df2['WTSD'] == 'Yes'].groupby('Seats').Won.value_counts().groupby(level=0).apply(lambda x: x/x.sum()*100).loc(axis=0)[slice(None), 'Yes'].reset_index(drop=True, level=1).reset_index(name='Won%')
...
#Aggression
#dfAGG = df2.replace([np.inf, -np.inf, 0.0], np.NaN).dropna(subset = ['Agg']).groupby('Seats').Agg.mean()
#
#dfAGG_Flop = df2.replace([np.inf, -np.inf, 0.0], np.NaN).dropna(subset = ['Agg_Flop']).groupby('Seats').Agg_Flop.mean()
#dfAGG_Turn = df2.replace([np.inf, -np.inf, 0.0], np.NaN).dropna(subset = ['Agg_Turn']).groupby('Seats').Agg_Turn.mean()
#dfAGG_River = df2.replace([np.inf, -np.inf, 0.0], np.NaN).dropna(subset = ['Agg_River']).groupby('Seats').Agg_River.mean()
#Winnings
...
df_Vpip = reduce(lambda left, right: pd.merge(left, right, on = 'Seats', how = 'outer'), dfs)

df_Vpip.to_csv(r'C:\Users\Adam\Desktop\Python\Master\df_Vpip.csv', index = False)
